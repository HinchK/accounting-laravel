...
final class RecordProjectProfitability
{
    ...
    {
        $start=CarbonImmutable::parse((string)$attributes['period_start']);$end=CarbonImmutable::parse((string)$attributes['period_end']);
        if($end->lessThan($start)) throw new InvalidProfitability('The profitability period must end on or after it starts.');
        foreach(['revenue_amount','cost_amount','estimate_amount','committed_amount','actual_amount','unbilled_wip_amount','billed_amount'] as $field){if(isset($attributes[$field])&&(float)$attributes[$field]<0)throw new InvalidProfitability($field.' cannot be negative.');}
