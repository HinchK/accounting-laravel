...
final class ProjectProfitability extends Model
{
    ...
    /** @return BelongsTo<ProjectJob, $this> */ public function projectJob():BelongsTo{return $this->belongsTo(ProjectJob::class,'project_job_id');}
    public function marginAmount():float{return (float)$this->revenue_amount-(float)$this->cost_amount;}
    public function marginPercent():float{return (float)$this->revenue_amount===0.0?0.0:($this->marginAmount()/(float)$this->revenue_amount)*100;}
    public function realizationPercent():float{return (float)$this->estimate_amount===0.0?0.0:((float)$this->billed_amount/(float)$this->estimate_amount)*100;}
}
