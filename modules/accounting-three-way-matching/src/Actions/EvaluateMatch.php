...
final class EvaluateMatch
{
    public function handle(array $attributes): MatchRecord
    {
        return DB::transaction(function () use ($attributes): MatchRecord {
            ...
            $exceptions = [];
            $ordered = (float) $attributes['ordered_quantity']; $received = (float) $attributes['received_quantity']; $billed = (float) $attributes['billed_quantity'];
            $orderedPrice = (float) $attributes['ordered_unit_price']; $billedPrice = (float) $attributes['billed_unit_price'];
            $expectedTax = (float) $attributes['expected_tax']; $billedTax = (float) $attributes['billed_tax'];
            if ($received > $ordered + $quantityTolerance) $exceptions[] = ['kind'=>'received_exceeds_ordered','expected_value'=>$ordered,'actual_value'=>$received,'tolerance'=>$quantityTolerance,'severity'=>ExceptionSeverity::Blocking];
            if ($billed > $received + $quantityTolerance) $exceptions[] = ['kind'=>'billed_exceeds_received','expected_value'=>$received,'actual_value'=>$billed,'tolerance'=>$quantityTolerance,'severity'=>ExceptionSeverity::Blocking];
            if ($billed < $received - $quantityTolerance) $exceptions[] = ['kind'=>'partial_quantity','expected_value'=>$received,'actual_value'=>$billed,'tolerance'=>$quantityTolerance,'severity'=>ExceptionSeverity::Warning];
            if (abs($billedPrice - $orderedPrice) > $priceTolerance) $exceptions[] = ['kind'=>'price_variance','expected_value'=>$orderedPrice,'actual_value'=>$billedPrice,'tolerance'=>$priceTolerance,'severity'=>ExceptionSeverity::Blocking];
            if (abs($billedTax - $expectedTax) > $taxTolerance) $exceptions[] = ['kind'=>'tax_variance','expected_value'=>$expectedTax,'actual_value'=>$billedTax,'tolerance'=>$taxTolerance,'severity'=>ExceptionSeverity::Blocking];
            ...
            foreach ($exceptions as $exception) $match->exceptions()->create(array_merge($exception, ['status'=>ExceptionStatus::Open]));
