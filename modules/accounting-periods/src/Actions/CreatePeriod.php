...
final class CreatePeriod
{
    ...
    public function handle(array $attributes): AccountingPeriod {
        return DB::transaction(function () use ($attributes): AccountingPeriod {
            if ($attributes['starts_on'] > $attributes['ends_on']) throw new \InvalidArgumentException('Period start must not be after its end.');
            if (isset($attributes['posting_ends_on']) && $attributes['posting_ends_on'] > $attributes['ends_on']) throw new \InvalidArgumentException('Posting end must not be after the period end.');
            ...
            if ($overlap) throw new \InvalidArgumentException('Accounting periods may not overlap within a book.');
