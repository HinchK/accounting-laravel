...
use Liberu\Accounting\Forecasts\Models\{Forecast, ForecastActual, ForecastLine};
...
final class ReplaceActual
{
    public function handle(Forecast $forecast, array $a): ForecastActual
    {
        ...
        foreach (['period_ref', 'source_ref'] as $k) if (blank($a[$k] ?? null)) throw new InvalidForecast("Missing actual field [{$k}].");
        if ($value < 0) throw new InvalidForecast('Actual value cannot be negative.');
        return DB::transaction(function () use ($forecast, $a, $value): ForecastActual {
            ...
            if ($line) $line->update(['actual_value' => $value, 'variance_value' => round((float) $line->forecast_value - $value, 2)]);
            return $actual;
        });
    }
}
