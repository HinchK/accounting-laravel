...
final class SaveFiscalCalendar
{
    ...
    {
        if ($attributes['starts_on'] > $attributes['ends_on']) throw new InvalidFiscalCalendar('Fiscal calendar start must not be after its end.');
        return DB::transaction(function () use ($calendar, $attributes): FiscalCalendar {
            ...
            if ($calendar) $query->where('id', '!=', $calendar->getKey());
            if ($query->exists()) throw new InvalidFiscalCalendar('Fiscal calendar periods may not overlap within a book.');
            $wasRecentlyCreated = $calendar === null;
            $calendar ??= new FiscalCalendar();
            $calendar->fill($attributes)->save();
            if ($wasRecentlyCreated) DB::afterCommit(fn () => $this->events->dispatch(new FiscalCalendarCreated($calendar)));
