...
class PolicyRule extends Model
{
    ...
    public function appliesOn(\DateTimeInterface|string $date): bool { $value=$date instanceof \DateTimeInterface?$date->format('Y-m-d'):$date; return $this->is_active && $value >= $this->effective_from->toDateString() && ($this->effective_until===null || $value <= $this->effective_until->toDateString()); }
}
