...
final class MatchRecordResource extends Resource
{
    ...
    public static function table(Table $table): Table { return $table->columns([TextColumn::make('purchase_order_id')->label('PO')->searchable(), TextColumn::make('receipt_id')->label('Receipt')->searchable(), TextColumn::make('bill_id')->label('Bill')->searchable(), TextColumn::make('status')->badge(), TextColumn::make('exceptions_count')->counts('exceptions'), TextColumn::make('created_at')->dateTime()->sortable()])->filters([SelectFilter::make('status')->options(['matched'=>'Matched','partial'=>'Partial','exception'=>'Exception','approved'=>'Approved','rejected'=>'Rejected'])])->defaultSort('created_at','desc'); }
    public static function getPages(): array { return ['index'=>Pages\ListMatches::route('/')]; }
}
