...
final class CreateJournal {
    public function handle(array $attributes, array $lines): JournalEntry { return DB::transaction(function() use($attributes,$lines) {
        if (empty($attributes['book_id']) || count($lines)<2) throw new InvalidJournal('A journal requires a book and at least two lines.');
        $book = Book::query()->find($attributes['book_id']);
        if ($book === null) throw new InvalidJournal('The journal book does not exist.');
        $accountIds = collect($lines)->pluck('account_id')->filter()->unique()->values();
        $accounts = Account::query()->whereIn('id', $accountIds)->get()->keyBy('id');
        if ($accounts->count() !== $accountIds->count() || $accounts->contains(fn (Account $account): bool => (int) $account->legal_entity_id !== (int) $book->getAttribute('legal_entity_id'))) throw new InvalidJournal('Every journal line must reference an account in the book legal entity.');
        ...
        $entry=JournalEntry::create($attributes+['journal_type'=>($attributes['journal_type']??JournalType::General->value),'status'=>JournalStatus::Draft->value]); $entry->lines()->createMany($lines); DB::afterCommit(fn()=>event(new JournalCreated($entry->fresh('lines')))); return $entry->load('lines');
    }); }
}
