...
Route::prefix('api/v1/accounting/financial-master-data')->middleware(['auth:sanctum', 'throttle:60,1'])->group(function (): void {
    ...
    foreach (['addresses', 'bank-details'] as $detail) {
        Route::get('parties/{party}/'.$detail, [PartyDetailController::class, 'index'])->defaults('detail', $detail);
        Route::post('parties/{party}/'.$detail, [PartyDetailController::class, 'store'])->defaults('detail', $detail);
        Route::delete('parties/{party}/'.$detail.'/{record}', [PartyDetailController::class, 'destroy'])->defaults('detail', $detail);
    }
    foreach (['items-services', 'tax-profiles', 'payment-terms'] as $resource) {
        Route::get($resource, [ReferenceDataController::class, 'index'])->defaults('resource', $resource);
        Route::get($resource.'/{record}', [ReferenceDataController::class, 'show'])->defaults('resource', $resource);
        Route::post($resource, [ReferenceDataController::class, 'store'])->defaults('resource', $resource);
        Route::match(['put', 'patch'], $resource.'/{record}', [ReferenceDataController::class, 'update'])->defaults('resource', $resource);
        Route::delete($resource.'/{record}', [ReferenceDataController::class, 'destroy'])->defaults('resource', $resource);
    }
});
