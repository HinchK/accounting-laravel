...
final class SupplierBill extends Model
{
    ...
    public function lines(): HasMany { return $this->hasMany(SupplierBillLine::class, 'bill_id'); }
    /** @return HasMany<SupplierBillCredit, $this> */
    public function credits(): HasMany { return $this->hasMany(SupplierBillCredit::class, 'bill_id'); }
    /** @return HasMany<SupplierBillDocument, $this> */
    public function documents(): HasMany { return $this->hasMany(SupplierBillDocument::class, 'bill_id'); }
    /** @return HasMany<SupplierBillMatch, $this> */
    public function matches(): HasMany { return $this->hasMany(SupplierBillMatch::class, 'bill_id'); }
    public function outstanding(): float { return max(0, (float) $this->total - (float) $this->credits()->sum('amount') - (float) $this->amount_paid); }
    public function isOverdue(?\DateTimeInterface $asOf = null): bool { return $this->due_on !== null && $this->outstanding() > 0 && $this->due_on->isBefore($asOf ?? now()); }
    public function scopeOpen(Builder $query): Builder { return $query->whereIn('status', [SupplierBillStatus::Approved->value, SupplierBillStatus::Posted->value])->where('payment_status', '!=', PaymentStatus::Paid->value); }
}
