...
use Liberu\Accounting\GoodsAndServiceReceipts\Models\{Receipt, ReceiptLine};
...
final class AddReceiptLine
{
    public function handle(Receipt $receipt, array $a): ReceiptLine
    {
        ...
        foreach (['line_ref', 'description'] as $k) if (blank($a[$k] ?? null)) throw new InvalidReceipt("Missing line field [{$k}].");
        if ($received <= 0 || $price < 0 || ($ordered !== null && $ordered < 0)) throw new InvalidReceipt('Receipt quantities and price are invalid.');
        if ($receipt->status !== ReceiptStatus::Draft) throw new InvalidReceipt('Lines can only be added to draft receipts.');
