...
final class SupplierBillController extends Controller
{
    ...
    public function show(SupplierBill $supplierBill): SupplierBillResource { return new SupplierBillResource($supplierBill->load('lines','credits','documents','matches','party')); }

    public function store(Request $request, CreateSupplierBill $action): SupplierBillResource
    {
        $data = $request->validate($this->rules()); $lines = $data['lines']; unset($data['lines']);
    ...
    {
        $data = $request->validate(array_merge($this->rules(), ['bill_number'=>['sometimes','string','max:80']])); $lines = $data['lines']; unset($data['lines']);
        return new SupplierBillResource($action->handle($supplierBill, $data, $lines));
    }

    public function approve(SupplierBill $supplierBill, ApproveSupplierBill $action): SupplierBillResource { return new SupplierBillResource($action->handle($supplierBill, auth()->id())); }
    public function post(SupplierBill $supplierBill, PostSupplierBill $action): SupplierBillResource { return new SupplierBillResource($action->handle($supplierBill)); }
    public function reject(Request $request, SupplierBill $supplierBill, RejectSupplierBill $action): SupplierBillResource { return new SupplierBillResource($action->handle($supplierBill, $request->validate(['reason'=>['required','string','max:2000']])['reason'])); }
    public function void(Request $request, SupplierBill $supplierBill, VoidSupplierBill $action): SupplierBillResource { return new SupplierBillResource($action->handle($supplierBill, $request->validate(['reason'=>['required','string','max:2000']])['reason'])); }
    ...
    { $data = $request->validate(['path'=>['required','string','max:1024'],'original_name'=>['required','string','max:255'],'mime_type'=>['required','string','max:128'],'sha256'=>['required','string','size:64'],'metadata'=>['nullable','array']]); $action->handle($supplierBill, $data); return new SupplierBillResource($supplierBill->refresh()->load('documents')); }

    public function duplicates(Request $request, DuplicateSupplierBillQuery $query): JsonResponse
    { $data = $request->validate(['party_id'=>['required','integer'],'bill_number'=>['required','string','max:80']]); return response()->json(['data'=>SupplierBillResource::collection($query->handle($data['party_id'], $data['bill_number']))]); }
    public function aging(Request $request, SupplierBillAgingQuery $query): JsonResponse { return response()->json(['data'=>$query->handle($request->integer('party_id') ?: null, $request->date('as_of'))]); }
