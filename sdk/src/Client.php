...
final class Client
{
    ...
    public function request(string $method, string $path, array $options = []): array
    {
        ...
        if ($status === 422) {
            throw new ValidationException($message, is_array($body['errors'] ?? null) ? $body['errors'] : []);
        }
        if ($status === 429) {
            throw new RateLimitException($message, (int) $response->getHeaderLine('Retry-After'));
        }
    ...
    public function generalLedger(): GeneralLedgerResource
    {
        return new GeneralLedgerResource($this);
    }
}
