...
use Liberu\Accounting\Mileage\Actions\{ApproveTrip,CaptureTrip,CreateVehicle,RecordMileageRate,ReimburseTrip,SubmitTrip};
...
it('rejects invalid and duplicate trip inputs', function (): void {
    ...
    $first=app(CaptureTrip::class)->handle($data);$same=app(CaptureTrip::class)->handle($data);expect($same->id)->toBe($first->id);
