...
use Liberu\Accounting\Leases\Actions\{CreateDisclosure,CreateLease,GenerateSchedule,ModifyLease,PostPayment};
use Liberu\Accounting\Leases\Enums\{LeaseStatus,PaymentStatus};
...
it('generates a lease schedule, posts payment, modifies, and discloses it', function (): void {
    ...
    $lease=app(GenerateSchedule::class)->handle($lease);$payment=$lease->payments->first();$posted=app(PostPayment::class)->handle($payment);$lease=$lease->fresh();$activeStatus=$lease->status;$modified=app(ModifyLease::class)->handle($lease,['modification_ref'=>'MOD-1','effective_date'=>'2026-06-01','new_payment_amount'=>1100,'reason'=>'Renewal']);$disclosure=app(CreateDisclosure::class)->handle($modified,'2026-06-30');
...
it('rejects invalid lease terms and premature payment posting', function (): void {
    ...
    $lease=app(CreateLease::class)->handle(['lease_ref'=>'LEASE-2','lessor_ref'=>'l','commencement_date'=>'2026-01-01','end_date'=>'2026-02-01','currency'=>'GBP','payment_amount'=>100,'useful_life_months'=>12]);expect(fn (): mixed => app(PostPayment::class)->handle($lease->payments()->create(['payment_ref'=>'p','due_date'=>'2026-02-01','amount'=>100])))->toThrow(InvalidLease::class);
});
