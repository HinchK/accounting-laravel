...
check_namespace() {
    local namespace="${1:-accounting-erp-laravel}"
...
validate_deployment() {
    ...
    if kubectl get deployment -n "$namespace" accounting-erp-laravel &> /dev/null; then
        local replicas ready available
        replicas=$(kubectl get deployment -n "$namespace" accounting-erp-laravel -o jsonpath='{.status.replicas}')
        ready=$(kubectl get deployment -n "$namespace" accounting-erp-laravel -o jsonpath='{.status.readyReplicas}')
        available=$(kubectl get deployment -n "$namespace" accounting-erp-laravel -o jsonpath='{.status.availableReplicas}')
        ...
        if [ "${ready:-0}" -eq "$replicas" ] && [ "${available:-0}" -eq "$replicas" ]; then
            ...
            kubectl get pods -n "$namespace" -l app=accounting-erp-laravel
            kubectl get events -n "$namespace" --sort-by='.lastTimestamp' | tail -10
        fi
    else
        log_warning "Deployment 'accounting-erp-laravel' not found in namespace '$namespace'"
    fi
}
